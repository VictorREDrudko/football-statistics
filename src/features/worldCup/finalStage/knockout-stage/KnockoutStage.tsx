...
export const KnockoutStage = ({matches}: KnockoutStage) => {
  ...
  const uniqueKnockoutStageTitle = matches ? Array.from(new Set(matches.map(match => match.stage))).sort() : []
  // Unique name teams in play-off
  const uniqueKnockoutStageTeams = matches ? Array.from(new Set(matches.map(match => match.teams).flat())) : []
